                    ...
                    <div class="mg-tp-15 flex-r just-between">
                        ...
                        <div class="add">
                            ...
                            <el-button type="primary" size="small" icon="el-icon-plus"
                                       @click="addProvider(teamUrl, projectCode, sheetCode)">添加数据
                            </el-button>
                        </div>
                    </div>
                </div>
                    ...
                    <el-table
                        ...
                        <template>
                            <el-table-column v-for="(item, index) in fieldList"
                                             :show-overflow-tooltip="item.type != '富文本'" :label="item.name"
                                             :key="index">
                                <template slot-scope="scope">
                                    <span v-if="item.type == '单行文本' && item.key != 'img'"
                                          v-html="scope.row[item.key]"></span>
                                    <span v-if="item.type == '多行文本' && item.key != 'img'"
                                          v-html="scope.row[item.key]"></span>
                                    <span class="rich-text" v-if="item.type == '富文本'"
                                          v-html="scope.row[item.key]"></span>
                                    ...
                                    <img class="table-img" v-if="item.type == '附件' && item.key == 'img'"
                                         v-for="url in scope.row[item.key]" :src="url.url + '!icon.jpg'"/>
                                    <img
                                    ...
                                    <el-dialog title="视频预览" width="50%" append-to-body top="20px"
                                               :visible.sync="dialogVisible" @close="close">
                                        <video width="100%" autoplay="autoplay" :src="playvideo" :poster="playvideo"
                                               controls="controls" id="video" preload ref="vueRef"></video>
                                    </el-dialog>
                                </template>
                            </el-table-column>
                        </template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getRecordList() {
        console.log(this.projectCode, '我执行了')
        this.providerList = []
        ...
        getRecordList(params, this.teamUrl, this.projectCode, this.sheetCode, this).then(res => {
          if (res.data.code === 1) {
            ...
            for (let fields of res.data.content) {
              fields.fields.recordId = fields.recordId
