<template>
  <el-col :xl='6' :lg="item.size !== 'large' ? 6 : 12" :md="item.size !== 'large' ? 6 : 12"
          :sm="item.size !== 'large' ? 12 : 24" :xs='24' v-if="item.type !== 'buttons'" class='search-item'>
    <el-form label-width='85px'>
      <el-form-item :label='item.label'>
        <el-input v-if="item.type === 'input'" v-model='item.value'
                  :placeholder="item.placeholder || '请填写' + item.label"
                  @on-enter='search'></el-input>
